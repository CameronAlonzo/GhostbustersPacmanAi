...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        "by the sum. "
        ##Normalize, by taking the keys and dividing them by the sum of all keys.
        ##It appears this exact method is actually in Util.py
        ##I didn't copy it but I guess I thought to tackle it the same way as they did.
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance, pacmanPosition, ghostPosition, jailPosition):
        ...
        """
        ##get the true distance between the due.
        trueDist = manhattanDistance(pacmanPosition, ghostPosition)
        ##do the special cases
        ##If ghost is in jail, return 1 with 100% certainty, else return 0
        if ghostPosition == jailPosition:
            return 1 if noisyDistance == None else 0
        elif noisyDistance == None:
            return 0
        
        return float(busters.getObservationProbability(noisyDistance, trueDist))
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()
        jail = self.getJailPosition()
        possible = self.beliefs
        ##should be quite fast
        for ghost in self.allPositions:
            ## For a while I tried multiplying by the self.beliefs[observation] but this was wrong. I tried this and it works? I'd guess its because since we are merging
            ## two probabilities in a cross product and then normalizing it that it would make sense?
            self.beliefs[ghost] = self.beliefs[ghost] * self.getObservationProb(noisyDistance, pacmanPosition, ghost, jail)
    ...
    def elapseTime(self, gameState):
        ...
        '''
        possible = self.beliefs.copy()
        ##set all the probabilties to 0
        for prob in self.allPositions:
            possible[prob] = 0
        ##Iterate thru and update them
        for ghost in self.allPositions:
            belief = self.beliefs[ghost]
            newPosDist = self.getPositionDistribution(gameState,ghost)
            for newG in newPosDist.keys():
                possible[newG] += belief * newPosDist[newG]
